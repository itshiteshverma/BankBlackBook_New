...
public class CustomAutoCompleteTextChangedListener_InsuredEvent implements TextWatcher {
    ...
    public void onTextChanged(CharSequence userInput, int start, int before, int count) {
        ...
        mainActivity.item = mainActivity.get_InsuredEvent_FromDb(userInput.toString());

        // update the adapater
        mainActivity.myAdapter_InsuredEvent.notifyDataSetChanged();
        mainActivity.myAdapter_InsuredEvent = new ArrayAdapter<String>(mainActivity, android.R.layout.simple_dropdown_item_1line, mainActivity.item);
        mainActivity.InsuredEvent.setAdapter(mainActivity.myAdapter_InsuredEvent);
